...
REM ============================================================
REM  Position Kiosk -- build & package script (build.bat)
REM
REM  Usage:
REM    Double-click, or run in cmd:
REM      build.bat            restore -&gt; test -&gt; publish -&gt; zip
REM      build.bat notest     skip tests, publish + zip only
REM
REM  Output:
...
if not exist "%SLN%" (
    ...
    echo         Please scaffold the project per Task 1 of the implementation plan.
    goto :fail
)
...
echo     3) Run PositionKiosk.exe --hash-password ^<password^> to set admin password
echo     4) Run install-shortcut.ps1 to create startup shortcut
echo ============================================================
echo.
pause
exit /b 0
